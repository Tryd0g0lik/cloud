...
from dotenv_ import APP_PORT, APP_PROTOKOL, APP_SERVER_HOST
    ...
    try:
        ...
        if ALLOWED_HOSTS:
            ...
            url_list = [
                (
                    view_url.replace(
                        f"{ALLOWED_HOSTS[0]}", f"{ALLOWED_HOSTS[0]}:{APP_PORT}"
                    )
                    if APP_PROTOKOL
                    else view_url
                )
            ...
            url_list = [
                (
                    view_url.replace(
                        f"{APP_SERVER_HOST}", f"{APP_SERVER_HOST}:{APP_PROTOKOL}"
                    )
                    if APP_PROTOKOL
                    else view_url
                )
        ...
        subject = render_to_string(
            template_name="email/activation_letter_subject.txt", context=_context
        )
        ...
        body_text = render_to_string(template_name=file_name, context=_context)
