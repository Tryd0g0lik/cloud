...
class FileStorageViewSet(viewsets.ViewSet):
    ...
    async def list(self, request, *args, **kwargs) -> JsonResponse:
        ...
        try:
                    ...
                    files.extend(
                        await sync_to_async(list)(
                            FileStorage.objects.filter(
                                user_id=int(getattr(request.COOKIES, "index"))
                            )
                        )
                    )
            ...
            # ERROR IN SCRIPT
            status_data = {
                "error": f"[{self.__class__.list.__name__}]: \
            Mistake => {e.__str__()}"
            }
        ...
        try:
            if request.user.is_authenticated:
                ...
                result_bool = await asyncio.create_task(
                    FileStorageViewSet.compare_twoFiles(
                        f"{time_path}/{file_obj.name}", int(user_ind), FileStorage
                    )
                )
                ...
                if not result_bool:
                    ...
                    file_old_list = await sync_to_async(list)(
                        FileStorage.objects.filter(original_name=f"{file_obj.name}")
                    )
    ...
    @action(detail=True, url_name="rename", methods=["post"])
    async def rename(self, request, **kwargs):
        ...
        try:
            ...
            if request.user.is_authenticated:
                ...
                file_record_list = await sync_to_async(list)(
                    FileStorage.objects.filter(id=int(file_id))
                )
                # CHECK THE NEW NAME IF EXISTS
                file_name_duplication = await sync_to_async(list)(
                    FileStorage.objects.filter(original_name=new_name)
                )
                ...
                if len(file_record_list) == 0:
                    return JsonResponse(
                        {"error": "Check the 'pk"}, status=status.HTTP_400_BAD_REQUEST
                    )
                # RENAME OF FILE
                new_file_path = (file_record_list[0].file_path.path).replace(
                    file_record_list[0].file_path.name.split("/")[-1], new_name
                )
    ...
    #
    @action(detail=True, url_name="comment", methods=["PATCH"])
        ...
        try:
            ...
            if request.user.is_authenticated:
                ...
                return await sync_to_async(JsonResponse)(
                    self.serializer_class(file_record_list[0]).data
                )
            else:
                # NOT LOGGED IN
    ...
    @action(detail=True, url_name="download", methods=["GET"])
    async def download(self, request, **wargs):
        try:
            # GET file  from db
            file_record_list = await sync_to_async(list)(
                FileStorage.objects.filter(special_link=wargs["pk"])
            )
            ...
            return JsonResponse(
                {"error": f"Mistake => {e.__str__()}"}, status=status.HTTP_404_NOT_FOUND
            )

    @action(detail=True, url_name="referral_links", methods=["GET"])
    async def referral_links(self, request, **kwargs):
        try:
            ...
            user_session_db = await sync_to_async(cache.get)(f"user_session_{user_ind}")
            # CHECK THE SESSION KEY of USER_SESSION  AND ADMIN adn CHECK USER ID
            if (
                user_session_db != user_session_client and not request.user.is_staff
            ) or (request.user.id != int(kwargs["pk"]) and not request.user.is_staff):
                response = JsonResponse(
                    {"data": ["User is not authenticated"]},
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            download_path = (
                f"/api/v1/files/{file_record_list[0].special_link}/download/"
            )
            ...
            response = cookie.empty_templete(
                "referral_link", referral_link, max_age_=30 * 1000
            )
    ...
    async def compare_twoFiles(
        link_ofNew_file: int, user_id: int, db_model: FileStorage
    ) -> bool:
        """
 ...
 the check will return the True.
        ...
        try:
            ...
            files_list = await sync_to_async(list)(
                db_model.objects.filter(user_id=user_id)
            )
            ...
            list_bool = [
                ...
                if hash_new_file == md5_chacker(view.file_path)
            ]
            if len(list_bool) > 0:
                return True
            return False
