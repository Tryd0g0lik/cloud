...
**Note:** 
backend и frontend должны располагаться в одном ядре. В backend и frontend распологася конфиг-файлы и код.
...
Пользователь и администратор по сценарию из '`cloud_user/views.py`' работают с:
- '`login`' и '`logout`'; 
- редактировать данные личного профиля;
- переименовать файл;
- поделиться ссылкой;
- генератор '`csrftoken`' '`{{baseUrl}}/users/`'. Сценарий для \
сравнения токена '`project/csrf.py::CustomCsrfMiddleware`':

Пользователь и администратор по сценарию из '`cloud_file/views.py`' работают с:
...
Администратор по сценарию из '`cloud/views.py`':
 ...
 - меняем статус пользователя '`{{baseUrl}}/admins/choice/:pk/change_status/`' (администратор или просто пользователь); 
...
1. Для сверки [csrf-токена используем](project/csrf.py) '`CustomCsrfMiddleware`' 
...
- '`FileStorageViewSet.change_status`' меняем статус пользователя.
...
- '`project/csrf.py::CustomCsrfMiddleware`' для работы с '`CSRFToken`' (with sync or async methods) 
...
## Интерфейс администратора c файлами
 ...
 - удалить одного или более пользователей (сводная таблица);
 - [x] '`Files`' количество файлов на сервере (сводная строка);
 - [x] '`Users`' количество пользователей на сервере (сводная строка);
 - [x] '`Downloaded`' количество файлов скаченных. Данное количество не указывает - сколько раз скачивался файл (сводная строка);
 ...
 - [x] '`User status'` - Статус пользователя (администратор или просто пользователь);
 - [x] '`Change status`' - Выбираем пользователей для изменения статуса (администратор или просто пользователь);
 - `'Delete'` -удалить одного или более пользователей (сводная таблица);
 - '`Save`' для сохранения выбора;
 - '`Add user`' При клике проходит релокация на страницу с формой для регистрации нового пользолвателя.
#### Note
- Загрузка файла в коллекцию пользователя, администратором не возможна.
- Удаление одного или более пользователей вместе с файлами, как из БД так и фактически из диска, проходит отдельным потоком. \
В не зависимости от количества пользователей и файлов, задержки в работе облака не происходят.  
...
[2025-02-09 15:24:17,9
85 985] trigger_reload autoreload : 266 INFO - E:\Netologe\diplom\backend\logs.py changed, reloading.
...
Ещё момент. 
Например такие методы  как `'UserView.patch_change'` объёмные по своему. \
По своей логике они универсальные/многозадачные.\
Конечно можно под каждую задачу сделать свой API, но тогда сам вес файла и код "раздуется" на проверках и прочее.
